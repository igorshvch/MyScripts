...
###Content=====================================================================
ARTICLE = r'Статья [0-9]{3}\. .+'
PARAGRAPH = r'[0-9]{1,2}\. .+'
DESICION_INDEX = r'[0-9]{1,2}\.[0-9]{1,2}\. .+'
DESICION_PURE = r'(?<=[0-9]\.[0-9]\. Вывод из судебной практики: ).*'
DESICION_ALT = (
    r'(?<=Вывод из судебной практики: По вопросу о).*(?=у судов нет единой позиции|существует две позиции судов|существует три позиции судов|существует четыре позиции судов|существует пять позиций судов)'
)
DESICION_CLEANED = (
...
def process_concls():
    ...
    while spl:
        ...
        if re.match(PATTERNS['par'], cursor):
            ...
            raise ValueError(
                'String didn\'t match to any pattern:'
