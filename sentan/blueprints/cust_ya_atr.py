...
FILE_NAMES = {
    ...
    'Влечет ли отказ в вычете подписание счета-фактуры неуполномоченным (неустановленным) лицом и (или) от имени лица, отрицающего причастность к деятельности контрагента (ст. 54.1, п. ': '102_2',
    'Вправе ли налогоплательщик принять к вычету НДС, уплаченный продавцу, применяющему освобождение по ст. 149 НК РФ (ст. 149, п. 2 ст. 171, пп. 2 п. 5 ст. 173 НК РФ) Налогоплательщик,': '518',
    'Вправе ли организация (индивидуальный предприниматель) применить вычет НДС без счета-фактуры (в частности, на основании кассового чека) при приобретении товаров (работ, услуг) за н': '105',
    'Должен ли налоговый орган при расчете недоимки учитывать сумму вычетов по НДС, не заявленную налогоплательщиком в декларации (п. 1 ст. 173, п. 1 ст. 80 НК РФ) Налоговый орган не до': '160_2',
    'Имеет ли налогоплательщик право на применение вычета, если он нарушил правила ведения книги покупок (продаж) или книга покупок (продаж) отсутствует (не представлена) (п. 1 ст. 171,': '109',
    'Может ли продавец принять к вычету НДС с полученного аванса на основании п. 5 ст. 171 НК РФ, если договор был расторгнут (изменен) и предоплата была зачтена в счет аванса по новому': '296',
    'Можно ли подтвердить право на вычет по НДС первичными документами, заверенными факсимильной подписью руководителя (п. 1 ст. 172 НК РФ) Заверенные факсимильной подписью документы не': '499',
    'Облагается ли НДС передача рекламных материалов, товаров (работ, услуг) в рекламных целях (пп. 1, 2 п. 1 ст. 146, пп. 25 п. 3 ст. 149 НК РФ) Передача товаров (работ, услуг) в рекла': '134',
    'Правомерно ли выставление арендодателем счета-фактуры арендатору в порядке возмещения расходов на оплату услуг организаций коммунального комплекса (ст. 171, п. 1 ст. 168 НК РФ) Выс': '121_2',
    'Правомерно ли применение освобождения от НДС по пп. 23.1 п. 3 ст. 149 НК РФ в отношении услуг застройщика при долевом строительстве жилых домов, в состав которых входят нежилые пом': '505',
    'Является ли нарушением ст. 169 НК РФ неуказание (неправильное указание) в счете-фактуре страны происхождения товара и (или) номера таможенной декларации по вине лица, которое ввезл': '91',
    'Является ли основанием для отказа в вычете НДС отсутствие (неверное указание) в счете-фактуре (корректировочном счете-фактуре) КПП продавца (покупателя) (п. п. 5, 5.1, 5.2 ст. 169,': '117_2'
}
...
class Scorer():
    ...
    def process_concl_pars_db(self,
        ...
        ###Paths
        path_to_concl = (
            self.DM.dir_struct['Concls'].joinpath(concl_dir_name, 'concls.dct')
        )
        print(path_to_concl)
        path_to_info = (
        ...
        concls = self.DM.load_pickle(str(path_to_concl))
        vocab_nw = self.DTL.load_vocab(spec='norm1', dir_name='2018-05-22')
