...
def worker(args):
    ...
    store.put(results, block=False)

def wrapper(concl_lemmed, store):
    local_cl = concl_lemmed
    local_store = store
    def inner_func(batch):
        all_args = local_cl, batch, local_store
        return worker(all_args)
    return inner_func
...
def main(raw_concl):
    ...
    store = Manager().Queue()
    gen = ((concl_lemmed, batch, store) for batch in acts_gen)
    proc_res_holder = []
    results = {
    ...
    with Pool(PROC_UNITS, start_process) as pool:
        t1 = time()
        end_time0 = time()-t0
        print_cust(
        ...
        proc_res_holder.append(
            pool.map_async(
                local_worker, gen
            )
        )
        for r in proc_res_holder: r.get()
    end_time1 = time()-t1
    print_cust(
        'Data processed! '
